...
//set up car speed and default lane
int lane = 1;
	...
	double current_speed_of_ego)
{
	...
	for(int i =0; i<number_of_lanes; i++){
		...
		for(int j = 0; j<closest_cars[i].size(); j++){
			double vx = closest_cars[i][j][3];
			double vy = closest_cars[i][j][4];
			double check_speed = sqrt(vx * vx + vy * vy);
			lane_speed.push_back(check_speed);
		}
		mean_speed_of_each_lane.push_back(accumulate(lane_speed.begin(), lane_speed.end(), 0.0)/lane_speed.size()); 
	}
	
	//Find closest vehicles to ego vehicle in each lane, ahead and behind ego vehicle
	vector<vector<vector<double>>> closest_vehicles;
	for(int i = 0; i<closest_cars.size(); i++){
		vector<vector<double>> temp_closest_vehicles;
		vector<vector<double>> cars_in_lane = closest_cars[i];
	...
	for(int i = 0; i<lane_change_heurestic.size(); i++){
		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2')
		{
			...
			if (s != "")
			{
				...
				if (event == "telemetry")
				{
					...
					//Vectors containing evenly spaced points 30m apart
					vector<double> ptsx;
					vector<double> ptsy;

					//Ref points for that keep track of the cars current location, or its previous
					//paths endpoint. Used to find the cars start point.
					double ref_x = car_x;
					double ref_y = car_y;
					double ref_yaw = deg2rad(car_yaw);

					//Check if cars previous path is empty, if so use current cars state
					//if not use the final endpoint for spline the new path forward
					if (prev_size < 2)
					{
					...
					//Frenet coordinates add evenly spaced points 30m apart from each other, so we can interspline them
					//using the spline.h header file and smoothen the cars trajectory
					...
					{
						//shift car ref angle to 0 degrees: i.e. to car local coordinates
					...
					//find way to break up all our reference points so we can travel at the required reference speed
					...
					for (int i = 1; i <= 50 - previous_path_x.size(); i++)
					{
						...
						//rotate back to map coordinate system
